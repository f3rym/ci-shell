...
type Settings struct {
	...
	// CacheDir — каталог данных утилиты (файл окружения, код джобы,
	// зеркала репозиториев), выбранный человеком (Фаза 11, GUIDE-02).
	// Пустое значение означает «пользователь ещё не выбирал»: формулу по
	// умолчанию знает internal/cache, а не этот пакет.
	CacheDir string `yaml:"cache_dir"`
	// Animation — вариант игрушки в строке заголовка кадра (Фаза 16,
	// TOY-01, idea-0.3.1 §1.1): одно из "ball", "wave", "belt", "cat",
	// "breathe", "off". Пустое значение означает не «выключено», а
	// «человек ещё не выбирал» — тем же приёмом, что CheckoutDir/CacheDir
	// выше: политику пустого значения (случайный выбор при каждом запуске)
	// знает вызывающий код (internal/ui/toy.go, parseToyKind), не этот
	// пакет.
...
// Update читает настройки, отдаёт их на правку переданной функции mutate и
// сохраняет целиком. Полная запись (Save выше) затирает соседние поля —
// точечная правка одного поля обязана быть операцией, а не
// последовательностью «читать, поправить, записать», повторённой в каждом
// вызывающем.
func Update(mutate func(*Settings)) (string, error) {
	s, _, err := Load()
	if err != nil {
		return "", err
	}
