...
// её сам, как раньше делал Shell. ctx нужен только для этой пробы — сама
// команда докера им намеренно НЕ управляется, см. ниже.
//
// Команда собирается через exec.Command, а не exec.CommandContext, хотя
// ctx уже есть под рукой: exec.CommandContext по умолчанию убивает процесс
// SIGKILL-ом, как только ctx отменяется, а docker exec -it без Setpgid
// наследует группу процессов самого ci — терминал доставляет Ctrl-C ВСЕЙ
// этой группе разом, то есть и docker exec, и ci получают SIGINT
// одновременно при любом Ctrl-C, нажатом ВНУТРИ контейнера (например, чтобы
// прервать одну зависшую команду — самое обычное действие в интерактивной
// сессии). Если бы команда была привязана к ctx, перехват SIGINT в ci
// (cmd/ci/main.go, signal.NotifyContext) отменял бы тот же ctx и убивал
// docker exec с ним заодно — одна попытка прервать команду внутри рушила
// бы всю сессию (CR-03 обзора v1.0.0). Без привязки к ctx этот сигнал
// доходит до docker exec ровно так же, как дошёл бы без всякого участия
// ci — терминал доставляет его обоим процессам напрямую, и внутри
// контейнера прерывается только команда, а не вся сессия.
func (c *Container) ShellCommand(ctx context.Context) (*exec.Cmd, error) {
	...
	return exec.Command(c.d.bin, "exec", "-it", "-w", c.workDir, c.ID, c.shell), nil
}
