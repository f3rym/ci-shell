...
func (m helpModel) renderKeys() string {
	...
	// 13-03; Фаза 18, KEYS-02): человек, открывший помощь, узнаёт правило
	// раньше перечня клавиш, а не только по перечислению одних символов без
	// объяснения. С Фазы 18 esc на самом левом крае больше не выходит —
	// формулировка это больше не обещает; q добавлен синонимом ← тем же
	// приёмом, что и ⏎ синонимом →. Все клавиши читаются из привязок
	// раскладки (k.Right.Help().Key и т. д.) — тот же приём, что и у
	// hintText() экрана помощи (Back) и у перечня ниже, — второго литерала
	// клавиши в файле не появляется: инвариант «ни одного имени клавиши
	// здесь не написано» остаётся в силе и для этой строки.
	k := m.keys
	law := m.theme.Muted.Render(fmt.Sprintf(
		"%s — глубже, %s — назад, %s — то же, что %s, %s — то же, что %s, %s — в начало ленты",
		k.Right.Help().Key, k.Left.Help().Key, k.Open.Help().Key, k.Right.Help().Key, k.Quit.Help().Key, k.Left.Help().Key, k.Back.Help().Key,
	))
