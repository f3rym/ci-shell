...
// Рисунок хранится БЕЗ отступа слева: отступ — свойство места, где логотип
// рисуют, а не самого рисунка. Пока отступ жил здесь, меню центрировало
// логотип вместе с ним и уводило блок вправо ровно на половину отступа,
// хотя подпись и пункты меню стояли по центру честно (L-08). Заставка
// добавляет свой отступ сама (splashLogoIndent ниже) и выглядит как прежде.
...
// splashLogoIndent — отступ логотипа на заставке. Ровно тот, что был вшит в
// сам рисунок до вынесения: заставка рисует логотип слева, а не по центру,
// и её вид этой правкой не меняется.
const splashLogoIndent = "                        "
...
// splashBackCmd — просьба вернуться в меню: заставка не хранит про меню
// ничего, кроме желания туда вернуться, — решение, что означает оверлей
// меню, принимает корневая модель. Единственная команда возврата с
// заставки (Фаза 18, KEYS-02: клавиша выхода q тоже ведёт сюда же, см.
// update ниже).
...
func (m splashModel) update(msg tea.Msg) (splashModel, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		// Поле ввода не открыто: клавиша возврата и клавиша q обе уводят в
		// меню (Фаза 18, KEYS-02) — заставка не единственная точка входа,
		// вернуться отсюда есть куда, и ни та ни другая больше не завершают
		// программу; выход остался на Cancel (ctrl+c), которую перехватывает
		// корневая модель раньше, чем сообщение доходит сюда.
...
func (m splashModel) applyCheckResult(msg checkResultMsg) (splashModel, tea.Cmd) {
	...
	if !msg.ok {
		...
		// С Фазы 18 (KEYS-02) q отсюда больше не выходит — возвращает в меню
		// тем же путём, что и остальные клавиши заставки (см. update выше);
		// формулировка называет и его, и настоящий выход.
		m.fatal = fmt.Sprintf(
			"не удалось продолжить: %s — %s вернёт в меню, %s выйдет",
			msg.reason, DefaultKeys().Quit.Help().Key, DefaultKeys().Cancel.Help().Key,
		)
		return m, nil
	}
...
// токена идёт единственной точкой сохранения ключа пакета интерфейса
// (saveToken, internal/ui/guide.go, Фаза 14) — прямого вызова token.Save
// здесь больше нет, второй экран (меню) просит о том же сохранении, и два
// прямых вызова разошлись бы на первой же правке. Успех — подсказка о
// сохранении с путём, неудача — подсказка о том, что токен не сохранён и
// используется только в этом запуске (то же решение, что принял вопрос в
// терминале). В обоих случаях заставка перезапускает ровно ту проверку,
// которая провалилась.
...
func (m splashModel) view(t Theme) string {
	...
	for _, line := range logoLines {
		if line != "" {
			b.WriteString(splashLogoIndent)
		}
		b.WriteString(line)
		b.WriteString("\n")
	}
