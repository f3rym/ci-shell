...
func (a App) updateInnerScreens(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		// 15-02, VAR-01 — план 20-02) открывается СВОЕЙ клавишей (KeyMap.Edit,
		// i), а не клавишей подтверждения. Пока правка висела на ⏎, она была
		// отступлением от закона ленты — единственным, но требующим оговорки
		// на каждом объяснении: «⏎ то же, что →, кроме одного вида одной
		// колонки». Отдельная клавиша снимает отступление целиком: ⏎ снова
		// значит ровно одно во всём интерфейсе, а правка перестаёт быть
		// исключением из навигации и становится тем, чем она и была, —
		// самостоятельным действием над строкой под курсором.
		// Предикаты сужены по виду одной и той же колонки (detailSecrets у
		// fillsSecret, detailEnv у fillsOverride), startFill разбирает оба.
		if a.overlay == overlayNone && !a.inputActive() && key.Matches(msg, a.keys.Edit) && (a.job.fillsSecret(a.ribbon.focusedID()) || a.job.fillsOverride(a.ribbon.focusedID())) {
			var cmd tea.Cmd
