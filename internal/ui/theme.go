...
const (
	...
	RowIconGap = 1
	// CursorReserve — ячейки, которые строка списка обязана оставить под
	// маркер курсора (пробел + GlyphCursor). Резервируется у КАЖДОЙ строки,
	// а не только у выбранной: ширина имени не должна прыгать при движении
	// курсора, а без резерва выбранная строка оказывалась ровно на эти две
	// ячейки шире колонки, и маркер переносился на следующую строку —
	// человек переставал видеть, на чём стоит (обзор v1.0.0, CR-02).
	CursorReserve = 2
...
// Signature, SoonNote — две именованные надписи входных экранов (Фаза 14,
// MENU-05, idea-0.3.1 §1, §2, §10). Обе рисуются приглушённым стилем — тем
// же, что и метаданные, — и символами замкнутого набора состояния выше не
// являются: это текст, а не символ.
//
// Signature — подпись автора, обязанная существовать в проекте ровно в
// одном экземпляре («ровно один раз, мелко» — прямое требование раздела 1
// idea-0.3.1). Единственное место, где она собирается в заголовок кадра, —
// internal/ui/menu.go, signatureTitle.
//
// SoonNote — пометка «скоро» для пунктов, которых ещё нет: сегодня GitHub в
// меню, завтра — список ключей, когда он появится.
const (
	Signature = "by F3"
	SoonNote  = "(скоро)"
)
...
const (
	...
	ColumnsVisibleMax = 3
	// RibbonChromeHeight — число строк кадра ВНЕ тела ленты: заголовок,
	// пустая строка перед телом, пустая строка после тела, строка клавиш,
	// пустая строка после неё, строка подсказки (App.viewInner,
	// internal/ui/app.go). Та же формула накладных строк кадра, что уже
	// применяет helpModel.setSize (internal/ui/help.go, `h := height - 6`)
	// для области просмотра экрана помощи — второй независимо выведенной
	// константы того же смысла в проекте не появляется, но обе остаются
	// раздельными литералами в разных файлах: helpModel.setSize не входит в
	// скоуп Фазы 16 (LOOK-01…02, план 16-01).
	RibbonChromeHeight = 6
	// CollapsedColumnWidth — ширина свёрнутой колонки ленты (Фаза 16, LOOK-03):
	// колонка левее видимого окна остаётся в кадре узкой полосой, а не
	// исчезает целиком (idea-0.3.1 §3, дословная просьба «убирать лишнее», не
	// «прятать совсем»). Рамка (по 1 ячейке слева и справа) плюс одна ячейка
	// содержимого — минимум, который вообще можно обвести рамкой Lip Gloss и
	// всё ещё отличить от соседней колонки. Полоса нарочно НЕ несёт текста:
	// одной ячейки недостаточно ни для одной графемы заголовка колонки без
	// обрезки до бессмысленного обрывка — она сигнализирует своим
	// присутствием и рамкой, что здесь есть свёрнутая колонка, а не пустота.
	CollapsedColumnWidth = 3
	// ColumnBodyOverhead — сколько строк высоты колонки уходит на то, что не
	// тело: заголовок колонки (1 строка) + рамка (2, верх и низ) + строка
	// запаса — та же величина 10 (= RibbonChromeHeight+4), которую
	// treeModel.setSize и jobsModel.setSize уже применяли буквальным
	// литералом до этой фазы (Фаза 16, план 16-01). Именованная константа
	// на все три модели-колонки разом — jobModel была единственной, что не
	// вычитала эту поправку вовсе (WR-03 обзора v0.3.1), и именно
	// отсутствие общей константы позволило расхождению остаться
	// незамеченным (Info-находка того же обзора).
	ColumnBodyOverhead = RibbonChromeHeight + 4
)
