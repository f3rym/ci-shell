...
// Размеры экрана дерева групп и проектов (Фаза 10, BROW-01): при 80 ячейках
// доступно 76 за вычетом отступов от краёв и зазора между панелями, доля
// даёт левой меньше минимума, значит левая берёт минимум, а правой остаётся
// больше её минимума — обе панели помещаются без отдельного порога
// схлопывания, тем же расчётом, что уже применён к экрану джобы.
const (
	// TreePanelMin — минимальная ширина левой панели (дерево).
	TreePanelMin = 24
	// RunsPanelMin — минимальная ширина правой панели (пайплайны).
	RunsPanelMin = 44
	// TreeSharePercent — доля левой панели от доступной ширины.
	TreeSharePercent = 30
)

// LogPanelLines — сколько строк лога джобы настоящего прогона видно в
// панели под списком джоб (Фаза 10, BROW-04). При высоте 24 на заголовок
// кадра, пустые строки, строку клавиш и строку подсказки уходит семь строк —
// панели остаётся достаточно и на список, и на шесть строк лога с рамкой.
const LogPanelLines = 6
