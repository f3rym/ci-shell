...
// (KEYS-02) q тоже уводит в меню (тем же путём, что и esc) — единственный
// путь выхода остался на Ctrl-C, которую перехватывает корневая модель.
// Клавиши читаются из раскладки (LOOK-04), а не литералом.
func HintSplashExit() string {
	return fmt.Sprintf(
		"%s снимет набранное, ещё раз — в меню; %s тоже в меню, %s выходит",
