...
// (LOOK-04), а не литералом. С Фазы 18 (KEYS-02) `q` на этом экране не
// выход, а шаг назад — формулировка называет настоящий выход (`Cancel`/
// `ctrl+c` или команда `:q`, обе доступны на экране джобы).
func HintCommitted() string {
	return fmt.Sprintf("зафиксировано — %s или :q для выхода, или продолжайте чинить следующий шаг", DefaultKeys().Cancel.Help().Key)
}
...
// не ради достижимого пути. С Фазы 18 (KEYS-02) esc/q с самой левой колонки
// ленты больше не выходят — выход остался на Cancel.
// Клавиша читается из раскладки (LOOK-04), а не литералом.
func HintTreeNoKeys() string {
	return fmt.Sprintf("ключей пока нет — %s выйдет из утилиты", DefaultKeys().Cancel.Help().Key)
}
...
// единственная точка входа, вернуться отсюда есть куда. С Фазы 18
// (KEYS-02) q ведёт себя как esc на уже пустом поле — уводит в меню, не
// выход; настоящий выход остался только на Cancel. Формулировка точна для
// экрана проверок целиком; пока открыто поле ввода ссылки, q, как и любая
// другая буква, просто печатается в поле — известное и принятое ограничение
// общей подписи на два состояния экрана (не относится к LOOK-04: это не
// клавиша, называющая себя неверно, а один и тот же текст под двумя разными
// состояниями поля ввода).
// Клавиши читаются из раскладки (LOOK-04), а не литералом.
func HintSplashExit() string {
	return fmt.Sprintf(
		"%s снимет набранное, ещё раз — в меню; %s тоже в меню; %s выйдет из утилиты",
...
// (KEYS-02) esc/q отсюда больше никуда не ведут — дальше двигаться некуда,
// а выход остался только на Cancel, доступной с любого экрана.
func HintColumnLeftmost() string {
	return DefaultKeys().Cancel.Help().Key + " выйдет из утилиты"
}
