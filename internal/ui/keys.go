...
func DefaultKeys() KeyMap {
	return KeyMap{
		...
		Back: key.NewBinding(
			key.WithKeys("esc"),
			// esc уводит в самую левую колонку ленты (Фаза 13); с Фазы 18
			// (KEYS-02) из неё больше НЕ выходит.
			key.WithHelp("esc", "в начало"),
		),
		...
		// Quit — с Фазы 18 (KEYS-02) q синоним стрелки влево, та же самая
		// функция шага назад (navShallower, internal/ui/ribbon.go), не
		// выход; поле сохраняет имя Quit (переименование расползлось бы по
		// пакету без изменения поведения), но подпись называет его новую
		// роль честно — выход остался на Cancel (ctrl+c) и на команде :q.
		Quit: key.NewBinding(
			key.WithKeys("q"),
			key.WithHelp("q", "то же, что "+GlyphLeft),
		),
...
// она остаётся и там названа синонимом. С Фазы 18 (KEYS-02) Quit — тем же
// приёмом синоним Left, и по той же причине в короткую форму не входит;
// вместо неё — Cancel, единственная клавиша, которая ДЕЙСТВИТЕЛЬНО выходит.
// Реализация интерфейса помощи компонента Bubbles.
//
...
//	"→ глубже"      → 1+1+6 = 8
//	"← назад"       → 1+1+5 = 7
//	"esc в начало"  → 3+1+8 = 12
//	"? помощь"      → 1+1+6 = 8
//	"ctrl+c выход"  → 6+1+5 = 12
//	5 разделителей " · " → 5*3 = 15
//	итого: 8+8+7+12+8+12+15 = 70 <= 78
func (k KeyMap) ShortHelp() []key.Binding {
	return []key.Binding{k.Up, k.Right, k.Left, k.Back, k.Help, k.Cancel}
}
...
// никогда не этой парой (см. KeyBar ниже). Cancel вместо Quit (Фаза 18,
// KEYS-02) — q больше не выход, пара «помощь и выход», навсегда видимая в
// каждой строке клавиш, обязана называть клавишу, которая ДЕЙСТВИТЕЛЬНО
// выходит; HintAs даёт короткое описание «выход» вместо полного
// Cancel.Help().Desc («отменить операцию или выйти») ради бюджета ширины
// строки клавиш (keyBarAvail) — полное описание остаётся на экране помощи,
// куда попадает через FullHelp() без изменений.
func MetaHints(k KeyMap) []KeyHint {
	return []KeyHint{Hint(k.Help), HintAs(k.Cancel, "выход")}
}
