...
type jobModel struct {
	...
	// columnView, посчитанный лентой (App.visibleLayout) — второго расчёта
...
//
// Накладные расходы колонки вычитаются здесь ВПЕРВЫЕ (Фаза 17, чинит WR-03):
// treeModel.setSize/jobsModel.setSize уже вычитали ColumnBodyOverhead —
// jobModel была единственной моделью-колонкой, что хранила сырую высоту
// терминала как есть, из-за чего logDetailHeight() (не меняется этой
// правкой — она уже вычитает 1 строку из m.height, а m.height теперь САМ уже
// поправлен) ошибочно давал встроенному просмотрщику лога шага примерно на 8
// строк больше, чем реально помещается внутри рамки колонки.
func (m jobModel) setSize(height int) jobModel {
	h := height - ColumnBodyOverhead
	if h < 0 {
		h = 0
	}
	m.height = h
